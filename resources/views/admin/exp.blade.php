                ...
                <table id="zero-config" class="table table-striped" style="width:100%">
                    ...
                    @foreach($experiences as $data)
                        <tr>
                            ...
                            <td>
                                <form method="POST" action="{{ route('edu.destroy', [$data->id]) }}">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
                                    <div class="btn-group" role="group">
{{--                                        <button onclick="location.href=''" type="button" class="btn btn-dark text-primary"> <i class="fas fa-external-link"></i></button>--}}
                                        <button onclick="location.href='{{ route('edu.edit', [$data->id]) }}'" type="button" class="btn btn-dark text-secondary"> <i class="fas fa-edit"></i></button>
